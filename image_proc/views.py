...
def create(request):
    if request.method == 'POST':
        ...
        with Image.open(new_med_img.proc_img) as img:
            ...
            edges = img_proc.filter(ImageFilter.FIND_EDGES)
...
def delete(request, pk):
    ...
    os.remove(os.path.join(settings.MEDIA_ROOT, med_img.stock_img.path))
    os.remove(os.path.join(settings.MEDIA_ROOT, med_img.proc_img.path))
    med_img.delete()
    return redirect(reverse('index'))
